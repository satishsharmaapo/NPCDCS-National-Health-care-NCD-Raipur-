...
import npcdcs.database.pojo.NcdAtChcReporting;

public class NcdAtChcReportingDAO extends AbstractDAO{
	

	@Override
	public List list() {
		ArrayList<NcdAtChcReporting> list=new ArrayList<NcdAtChcReporting>();
		// see the query ----------------------
		   ...
		   try{
			   ...
			   while(rs.next())
			   {   
				   NcdAtChcReporting nr=new NcdAtChcReporting();                                     
				   
				   
				   nr.setNoofcancer(rs.getInt(2));		     
				   nr.setNoofcardiovascular(rs.getInt(8));
				   nr.setNoofdiabetes(rs.getInt(8));
				   nr.setNoofhypertension(rs.getInt(8));
				   nr.setNoofperson(rs.getInt(8));
				   nr.setNoofreftodistrict(rs.getInt(8));
				   nr.setNoofhomevisit(rs.getInt(8));
